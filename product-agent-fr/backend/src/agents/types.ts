// ─── Types I/O de chaque agent ────────────────────────────────────────────────

export interface ProduitInput {
  nom: string
  description: string
  url?: string
  prix?: number
  categorie?: string
  imageBase64?: string
  imageMimeType?: string
}

// ─── Agent 1 : Analyse Produit ────────────────────────────────────────────────
export interface AnalyseProduit {
  nom: string
  categorie: string
  benefice_principal: string
  probleme_resolu: string
  caracteristiques_uniques: string[]
  cible_principale: string
  fourchette_prix: { min: number; max: number; recommande: number }
  mots_cles_concurrents: string[]
  facteur_wow: string
  potentiel_viral: 'faible' | 'moyen' | 'élevé'
  est_produit_gagnant: boolean
}

// ─── Agent 2 : Analyse Marché FR ─────────────────────────────────────────────
export interface AnalyseMarche {
  score_demande: number           // 0-100
  niveau_concurrence: 'faible' | 'moyen' | 'élevé'
  risque_saturation: 'faible' | 'moyen' | 'élevé'
  tendance: 'croissante' | 'stable' | 'déclinante'
  recherches_mensuelles_estimees: number
  marches_prioritaires: string[]  // régions FR + pays francophones
  saisonnalite: string
  fenetre_opportunite: string
  taille_marche_estime: string
  barrieres_entree: string[]
  insights_cles: string[]
  plateformes_fr_pertinentes: string[]  // Leboncoin, Vinted, etc.
}

// ─── Agent 3 : Avatar Client FR ──────────────────────────────────────────────
export interface AvatarClient {
  prenom: string
  age: { min: number; max: number }
  genre: string
  localisation: string[]
  revenu: string
  profession: string[]
  points_douleur: string[]
  desirs: string[]
  objections_typiques: string[]
  declencheurs_achat: string[]
  reseaux_sociaux: string[]
  habitudes_achat_fr: string[]   // spécifique au comportement FR
  valeurs: string[]
  journee_type: string
  moteurs_emotionnels: string[]
  sensibilite_prix: 'faible' | 'moyenne' | 'élevée'
  confiance_dropshipping: 'faible' | 'moyenne' | 'élevée'
}

// ─── Agent 4 : Stratégie Marketing ───────────────────────────────────────────
export interface StrategieMarketing {
  positionnement: string
  proposition_valeur_unique: string
  strategie_prix: string
  tunnel_de_conversion: Array<{
    etape: string
    objectif: string
    canal: string
    message_cle: string
  }>
  phases_lancement: Array<{
    phase: string
    duree: string
    budget: string
    objectif: string
    actions: string[]
  }>
  canaux: Array<{
    canal: string
    priorite: 'principal' | 'secondaire'
    budget_pourcent: number
    tactiques: string[]
  }>
  kpis: Array<{ metrique: string; objectif: string }>
  roas_estime: string
  budget_lancement_recommande: string
}

// ─── Agent 5 : Création d'Offre ───────────────────────────────────────────────
export interface CreationOffre {
  nom_offre: string
  prix_psychologique: number
  prix_barre: number           // prix barré (ancien prix)
  paiement_plusieurs_fois: {
    disponible: boolean
    options: string[]          // "3x sans frais", "4x Alma"
    mensualite: string
  }
  garantie: {
    duree: string
    type: string               // satisfait ou remboursé
    message: string
  }
  bonus_inclus: string[]
  urgence_rarete: string[]
  livraison: {
    delai: string
    gratuite_a_partir: string
    message_confiance: string
  }
  badge_confiance: string[]    // "Paiement sécurisé", "Retours gratuits"
  bundle_suggestions: Array<{
    nom: string
    prix: number
    economies: string
  }>
  appel_a_action_principal: string
}

// ─── Agent 6 : Page Produit FR ───────────────────────────────────────────────
export interface PageProduit {
  titre_principal: string
  sous_titre: string
  description_hero: string
  points_cles: string[]         // bullet points bénéfices
  preuves_sociales: string[]    // avis clients simulés
  faq: Array<{ question: string; reponse: string }>
  declencheurs_urgence: string[]
  appel_a_action: string
  titre_seo: string
  meta_description: string
  mots_cles_seo: string[]
  sections: Array<{ titre: string; contenu: string }>
  script_email_abandon_panier: string
}

// ─── Agent 7 : Publicités FR ─────────────────────────────────────────────────
export interface CampagnesPub {
  meta: {
    principales: Array<{
      format: string
      accroche: string
      texte_principal: string
      description: string
      appel_a_action: string
      audience: string
      objectif: string
      angle: string
    }>
  }
  tiktok: {
    pubs: Array<{
      accroche: string
      legende: string
      hashtags: string[]
      musique: string
      duree: string
      format: string
      angle_createur: string
    }>
  }
}

// ─── Agent 8 : Scripts Contenu FR ────────────────────────────────────────────
export interface PlanContenu {
  scripts_tiktok: Array<{
    angle: string
    accroche: string
    script: string
    duree: string
    appel_a_action: string
    type_createur: string      // "mère de famille", "entrepreneur", etc.
  }>
  accroches: string[]
  angles: string[]
  sequence_email: Array<{
    objet: string
    previsualisation: string
    corps: string
    appel_a_action: string
    delai: string
  }>
  calendrier_contenu: Array<{
    jour: number
    plateforme: string
    type: string
    sujet: string
  }>
}

// ─── Agent 9 : MASTER AGENT — Scoring + Décision ─────────────────────────────
export interface ScoreMaster {
  score_global: number          // 0-10
  verdict: 'gagnant' | 'test'  | 'risqué' | 'éviter'
  indicateur: '🟢' | '🟡' | '🔴'
  decision: string              // "LANCER", "TESTER", "AFFINER", "ABANDONNER"
  detail_scores: {
    demande_marche: number
    concurrence: number
    marge_potentielle: number
    potentiel_viral: number
    facilite_lancement: number
    adaptabilite_france: number
  }
  forces: string[]
  faiblesses: string[]
  plan_action_immediat: Array<{
    priorite: number
    action: string
    impact: 'élevé' | 'moyen' | 'faible'
    delai: string
    budget_estime: string
  }>
  chiffre_affaires_estime: {
    mois_1: string
    mois_3: string
    mois_6: string
  }
  seuil_rentabilite_unites: number
  conseil_final: string
}

// ─── Résultat complet du pipeline ─────────────────────────────────────────────
export interface ResultatPipeline {
  id: string
  input: ProduitInput
  cree_le: string
  produit: AnalyseProduit
  marche: AnalyseMarche
  avatar: AvatarClient
  strategie: StrategieMarketing
  offre: CreationOffre
  page: PageProduit
  pubs: CampagnesPub
  contenu: PlanContenu
  score: ScoreMaster
}
