...
import type { AnalyseProduit, AvatarClient, StrategieMarketing, CreationOffre } from './types'
...
export async function runCreationOffre(
  produit: AnalyseProduit,
  avatar: AvatarClient,
  strategie: StrategieMarketing
): Promise<CreationOffre> {
...
Déclencheurs achat : ${safeArr(avatar.declencheurs_achat).join(' | ')}
...
Retourne un objet JSON avec ce schéma exact :
{
  ...
  return appelAgent<CreationOffre>(SYSTEME, prompt, 2000)
}
